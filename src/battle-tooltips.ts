...
class BattleTooltips {
	...
	static runClickActionOfButtonWithTooltip = false;
	...
	lockTooltip() {
		if (BattleTooltips.elem && !BattleTooltips.isLocked) {
			...
			BattleTooltips.elem.className += ' tooltip-locked';
		}
	}
	...
	listen(elem: HTMLElement) {
		...
		$elem.on('click', '.has-tooltip', this.showTooltipEvent);
		$elem.on('touchstart', '.has-tooltip', this.longLockTooltipEvent);
	 ...
	 *
	 * (Currently only touchstart)
	 */
	longLockTooltipEvent = (e: Event) => {
		this.showTooltip(e.currentTarget as HTMLElement);
	...
	showMoveTooltip(move: Move, isZ: boolean, pokemon: Pokemon, serverPokemon: ServerPokemon) {
		...
		if (move.id === 'naturepower') {
			...
			let calledMove = Dex.getMove(calls);
	...
	showPokemonTooltip(clientPokemon: Pokemon | null, serverPokemon?: ServerPokemon | null, isActive?: boolean) {
		...
		if (pokemon.fainted) {
			...
			if (serverPokemon) {
				text += '<p>HP: ' + serverPokemon.hp + '/' + serverPokemon.maxhp + (pokemon.status ? ' <span class="status ' + pokemon.status + '">' + pokemon.status.toUpperCase() + '</span>' : '') + '</p>';
			}
		} else {
			let exacthp = '';
			if (serverPokemon) {
			...
			text += '<p>HP: ' + Pokemon.getHPText(pokemon) + exacthp + (pokemon.status ? ' <span class="status ' + pokemon.status + '">' + pokemon.status.toUpperCase() + '</span>' : '');
			if (clientPokemon) {
		...
		let showOtherSees = !this.battle.hardcoreMode && isActive;
		if (serverPokemon) {
			...
			text += this.renderStats(clientPokemon, serverPokemon);
			if (showOtherSees) {
		...
		if (this.battle.gen > 2 && showOtherSees && !this.battle.tier.includes("Let's Go")) {
			if (!pokemon.baseAbility && !pokemon.ability) {
				let abilities = template.abilities;
				text += '<p>Possible abilities: ' + abilities['0'];
		...
		if (showOtherSees && clientPokemon) {
			let item = '';
			...
			if (template.baseStats) {
				...
				text += '<p>Spe ' + min + ' to ' + max + ' (before items/abilities/modifiers)</p>';
			}
		}
	...
	renderStats(clientPokemon: Pokemon | null, serverPokemon: ServerPokemon) {
	...
	getMoveType(move: Move, value: ModifiableValue): [TypeName, 'Physical' | 'Special' | 'Status'] {
		...
		if (move.flags['sound'] && value.abilityModify(0, 'Liquid Voice')) {
			moveType = 'Water';
		}
		const noTypeOverride = ['judgment', 'multiattack', 'naturalgift', 'revelationdance', 'struggle', 'technoblast', 'weatherball'];
		if (moveType === 'Normal' && category !== 'Status' && !noTypeOverride.includes(move.id)) {
